...
const port = 5001;
const resourceApiEndpont =
  process.env.RESOURCE_API_ENDPOINT || "http://localhost:5000";
const dynamodbTable = process.env.DYNAMODB_TABLE || "development-inventory";
const awsDefaultRegion = process.env.AWS_DEFAULT_REGION || "ap-southeast-1";
...
app.post("/register", (request, response) => {
  ...
  resourceApiUrl =
    resourceApiEndpont + "/get/" + inventoryRegistryJson.ResourceId;
    ...
    resp.on("end", () => {
      ...
      dynamoDb.put(params, function (err, data) {
        if (err) {
          ...
          response.status(201).json({ insertedId: inventoryRegistryJson._id });
        }
      });
    });
    resp.on("error", (err) => {
      if (resp.statusCode == 404) {
        response.status(404).json({ message: "The resource wasn't found." });
      } else {
        console.log(err);
      }
    });
...
app.get("/summary", function (request, response) {
  ...
  client = resourceApiListUrl.startsWith("http://")
    ? http_client
    : https_client;
    ...
    function (item, callback) {
      ...
      dynamoDb.scan(params, (err, data) => {
        ...
        dynamoDb.scan(paramsUnavailable, (err, data) => {
          ...
          tempObject.TotalCopies =
            tempObject.AvailableCopies + tempObject.UnavailableCopies;
